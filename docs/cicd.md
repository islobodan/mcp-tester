...
Every CI run uploads an HTML test report as a downloadable artifact:

- Generated by `jest-html-reporters` on every `npm test` run
- Saved to `reports/test-report.html`
- Uploaded per Node version (`test-report-node-18`, `test-report-node-20`, `test-report-node-21`)
- Retained for 14 days
- Includes: collapsible test tree, timing per test, failure details with stack traces
